...
public class AppUtil {
	...
	/**
	 * Construccion de sqlCount
	 * @param baseQuery
	 * @param paginationCriteria
	 * @return
	 */
	public static String buildCountQuery(String baseQuery, PaginationCriteria paginationCriteria) {
		StringBuilder sb = new StringBuilder("SELECT count(*) FROM (SELECT BASEINFO.* FROM ( #BASE_QUERY# ) BASEINFO #WHERE_CLAUSE#) FILTERED_ORDERD_RESULTS");
		String finalQuery = null;
		if(!AppUtil.isObjectEmpty(paginationCriteria)) {
			finalQuery = sb.toString().replaceAll("#BASE_QUERY#", baseQuery)
							.replaceAll("#WHERE_CLAUSE#", ((AppUtil.isObjectEmpty(paginationCriteria.getFilterByClause())) ? "" : " WHERE ") + paginationCriteria.getFilterByClause());
