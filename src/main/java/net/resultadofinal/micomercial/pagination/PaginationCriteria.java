package net.resultadofinal.micomercial.pagination;
...
public class PaginationCriteria {
		...
		if (!isFilterByEmpty()) {
			...
			while (fbit.hasNext()) {
				...
				if(null == fbsb) {
					...
					fbsb.append(SPACE)
							.append(BRKT_OPN)
								.append("upper(cast("+pair.getKey()+" as varchar))")
									.append(LIKE_PREFIX)
										.append(pair.getValue().toUpperCase())
											.append(LIKE_SUFFIX)
												.append(BRKT_CLS);
	...
	public String getOrderByClause() {
		...
		if(!isSortByEmpty()) {
			...
			if(!isFilterByEmpty()) {
				Iterator<Entry<String, String>> fbit = filterBy.getMapOfFilters().entrySet().iterator();
				while (fbit.hasNext()) {
					Entry<String, String> filterValue =  fbit.next();
			...
			while (sbit.hasNext()) {
				...
				if(null == sbsb) {
					...
					if(pair.getKey().equals("nombre") || pair.getKey().equals("generico") || pair.getKey().equals("xlaboratorio")) {
						sbsb.append(ORDER_BY_CASE)
						.append(WHEN +pair.getKey()).append(" LIKE '").append(value+"' THEN 1 ")
						.append(WHEN +pair.getKey()).append(" LIKE '").append(value+"%' THEN 2 ")
						.append(WHEN +pair.getKey()).append(" LIKE '%").append(value+"' THEN 4 ")
						.append("ELSE 3 END ");
					}else {
						if(pair.getKey()!=null && pair.getKey().toString().length()>0)
	...
	private static final String ORDER_BY_CASE = " ORDER BY CASE";
